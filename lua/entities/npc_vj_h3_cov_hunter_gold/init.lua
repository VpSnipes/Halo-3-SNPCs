...
ENT.RangeAttackEntityToSpawn = "obj_vj_h3_fuel_rod_hunter_proj" -- The entity that is spawned when range attacking
...
ENT.PoseParameterLooking_Names = {pitch={"head_aim_pitch", "body_aim_pitch", "crouch_aim_pitch", "unarmed_aim_pitch"},yaw={"head_aim_yaw", "body_aim_yaw", "crouch_aim_yaw", "unarmed_aim_yaw"},roll={}} -- Custom pose parameters to use, can put as many as needed
...
function ENT:RangeAttackCode_GetShootPos(TheProjectile)
	...
	return self:Trajectory(TheProjectile:GetPos(), aimAt+self:GetEnemy():GetVelocity()*(dist/2350), 2350)
end
---------------------------------------------------------------------------------------------------------------------------------------------
...
function ENT:CustomOnThink_AIEnabled()
	...
	if self.RangeAttacking == false && self.IsCrouching == false && self.MeleeAttacking == false && self.Dead == false && self:GetEnemy() != nil && self.VJ_IsBeingControlled == false	then
		...
		if EnemyDistance <= 2000 && math.random(1,30) == 1 && CurTime() > self.NextCrouchTime then -- Random movement
	...
	if self:GetEnemy() != nil && self.VJ_IsBeingControlled == false then	
		...
		if self:GetPos():Distance(self:GetEnemy():GetPos()) <= 2000 && math.random(1,30) == 1 && CurTime() > self.NextMoveTime && self:Visible(self:GetEnemy()) && self.CanMoveAgain == true && self.MeleeAttacking == false then
